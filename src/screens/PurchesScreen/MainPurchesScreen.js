...
export default function MainPurchesScreen({navigation}) {
    ...
    <Container
      ...
      {selectedItems.length > 0 && (
        <View style={styles.selectedItemsContainer}>
          <Text style={styles.selectedItemsTitle}>Selected Medicines</Text>
          <FlatList
            data={selectedItems}
            horizontal
            ...
            renderItem={({ item }) => (
              <View style={styles.selectedItem}>
                <Image source={{ uri: item.images[0]?.image }} style={styles.selectedItemImage} />
                <Text style={styles.selectedItemText} numberOfLines={1}>{item.title}</Text>
              </View>
            )}
            showsHorizontalScrollIndicator={false}
          />
      ...
      {/* Total items and Next button section */}
      {selectedItems.length > 0 && (
        <View style={styles.bottomSummaryContainer}>
          <View style={styles.totalItemsContainer}>
            <Text style={styles.totalItemsCountText}>{selectedItems.length} item</Text>
            <Text style={styles.totalItemsPriceText}>₹{selectedItems.reduce((sum, item) => {
              // Assuming each item in selectedItems has a price property and a quantity property
              // If item structure is different, this calculation needs adjustment.
              const selectedPack = item.size.find(pack => pack._id === selectedPackId);
              const price = selectedPack ? parseFloat(selectedPack.price.replace('₹', '')) : 0;
              return sum + price * (item.quntity || 1); // Assuming default quantity is 1 if not available
            }, 0).toFixed(2)}</Text>
          </View>
